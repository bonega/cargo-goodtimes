...
- Fix stack overflow when workspace has cyclic dev-dependencies.
- Dev-dependency edges are now excluded from the build graph since they don't affect compilation order.

## [0.3.0] - 2026-02-21

### Added

- Shareable links — frontend state is encoded in the URL hash so charts can be shared with others.

### Changed

- Replaced React with Preact, reducing JS bundle size by 84%.
- Moved the remove-dependency button outside the bar for better usability.
